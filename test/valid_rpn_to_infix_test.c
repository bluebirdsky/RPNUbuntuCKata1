...
START_TEST(When_Given_a_b_PLUS_Returns_a_PLUS_b_And_True) {
  char infix[INFIX_BUFFER_SIZE];
  const char rpn[] = "ab+";
...
START_TEST(When_Given_f_g_SUB_h_MULT_Returns_OBR_f_SUB_g_CBR_MULT_h_And_True) {
  char infix[INFIX_BUFFER_SIZE];
  const char rpn[] = "fg-h*";
...
START_TEST(When_Given_i_j_h_k_POW_DIV_SUB_Returns_i_SUB_OBR_j_DIV_OBR_h_POW_k_2CBR_And_True) {
  char infix[INFIX_BUFFER_SIZE];
  const char rpn[] = "ijhk^/-";
